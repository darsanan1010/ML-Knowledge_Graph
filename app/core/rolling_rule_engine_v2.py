...
try:
    ...
    def get_risk_level(score):
        if score >= 0: return "Low", 0
        elif score >= -2: return "Moderate", 0
        elif score >= -4: return "High", 0
        else: return "Critical", 0
    def score_to_confidence(score):
        return min(abs(score) / 10.0, 1.0)
...
try:
    ...
    def get_resident_baselines(*args, **kwargs):
        return pd.DataFrame()
from app.core.trend_engine_v2 import get_resident_trend_context


REFERENCE_ACTIVE_WINDOWS_PER_DAY = 16.0

# V2 Rule Engine Constants
HR_RECOVERY_MIN_THRESHOLD_BPM = 110.0
HR_RECOVERY_SLOPE_THRESHOLD = -5.0

SBP_PERSISTENT_MAX_THRESHOLD_MMHG = 110.0
SBP_DECLINE_SLOPE_THRESHOLD = -10.0

FALSE_MOBILITY_STEP_THRESHOLD = 500.0
...
def evaluate_rolling_risk_rules(row):
    ...
    if pd.notna(avg_hr) and pd.notna(ref_hr) and pd.notna(hr_tol) and ref_hr > 0:
        if avg_hr > ref_hr + hr_tol:
            pct_inc = min(100, ((avg_hr - ref_hr) / ref_hr) * 100)
            add_driver(
                drivers,
                "Vitals",
                f"Heart rate elevated {pct_inc:.0f}% above baseline over 6h",
                f"6-Hour average HR {avg_hr:.0f} bpm above baseline {ref_hr:.0f} bpm",
                -1,
                "30m_aggregation",
            )
        elif avg_hr < ref_hr - hr_tol:
            pct_dec = min(100, ((ref_hr - avg_hr) / ref_hr) * 100)
            add_driver(
                drivers,
                "Vitals",
                f"Heart rate reduced {pct_dec:.0f}% below baseline over 6h",
                f"6-Hour average HR {avg_hr:.0f} bpm below baseline {ref_hr:.0f} bpm",
                -1,
                "30m_aggregation",
            )
    ...
    if (
        ...
        hrv_drop = min(100, ((ref_hrv - avg_hrv) / ref_hrv) * 100)
        if hrv_drop > 25:
            add_driver(
                drivers,
                "Vitals",
                f"HRV reduced {hrv_drop:.0f}% below baseline over 6h",
                f"6-Hour average HRV {avg_hrv:.0f} below baseline {ref_hrv:.0f}",
                -2,
                "30m_aggregation",
            )
    ...
    if pd.notna(spo2_availability) and spo2_availability >= 0.25 and pd.notna(ref_spo2) and ref_spo2 > 0:
        if pd.notna(min_spo2) and min_spo2 < 92:
            pct_dec = min(100, ((ref_spo2 - min_spo2) / ref_spo2) * 100)
            add_driver(
                drivers,
                "Vitals",
                f"SpO2 reduced {pct_dec:.0f}% below baseline over 6h",
                f"6-Hour minimum SpO2 {min_spo2:.0f}% below baseline {ref_spo2:.0f}%",
                -2,
                "30m_aggregation",
            )
        elif pd.notna(avg_spo2) and avg_spo2 < ref_spo2 - 3:
            pct_dec = min(100, ((ref_spo2 - avg_spo2) / ref_spo2) * 100)
            add_driver(
                drivers,
                "Vitals",
                f"SpO2 reduced {pct_dec:.0f}% below baseline over 6h",
                f"6-Hour average SpO2 {avg_spo2:.0f}% below baseline {ref_spo2:.0f}%",
                -1,
                "30m_aggregation",
            )
    ...
    if pd.notna(avg_sbp) and pd.notna(ref_sbp) and ref_sbp > 0:
        if avg_sbp < 95:
            pct_dec = min(100, ((ref_sbp - avg_sbp) / ref_sbp) * 100)
            add_driver(
                drivers,
                "Vitals",
                f"Systolic blood pressure reduced {pct_dec:.0f}% below baseline over 6h",
                f"6-Hour average SBP {avg_sbp:.0f} mmHg below baseline {ref_sbp:.0f} mmHg",
                -2,
                "30m_aggregation",
            )
        elif pd.notna(sbp_tol) and avg_sbp < ref_sbp - sbp_tol:
            pct_dec = min(100, ((ref_sbp - avg_sbp) / ref_sbp) * 100)
            add_driver(
                drivers,
                "Vitals",
    ...
    steps_12h = row.get("steps_12h")
    steps_6h = row.get("steps_6h")
    ref_steps = row.get("reference_step_count")
    if pd.notna(ref_steps) and ref_steps > 0:
        ref_steps_12h = ref_steps * 0.50
        ref_steps_6h = ref_steps * 0.25
        activity_ratio_12h = row.get("activity_ratio_12h")
        
        if pd.notna(activity_ratio_12h) and activity_ratio_12h < 0.25:
            pct_decline = int((1.0 - activity_ratio_12h) * 100)
            pct_decline = max(0, min(100, pct_decline))
            if pd.notna(steps_12h):
                detail_str = f"12-Hour steps {steps_12h:.0f} below baseline {ref_steps_12h:.0f}"
            else:
                detail_str = f"12-Hour activity ratio {activity_ratio_12h:.2f} is low"
            add_driver(
                drivers,
                "Mobility",
                f"Mobility declined {pct_decline}% over 12h from baseline",
                detail_str,
                -2,
                "30m_aggregation",
            )
        elif pd.notna(steps_12h) and steps_12h < ref_steps_12h * 0.5:
            pct_decline = int((1.0 - (steps_12h / ref_steps_12h)) * 100)
            pct_decline = max(0, min(100, pct_decline))
            add_driver(
                drivers,
                "Mobility",
                f"Mobility declined {pct_decline}% over 12h from baseline",
                f"12-Hour steps {steps_12h:.0f} below baseline {ref_steps_12h:.0f}",
                -2,
                "30m_aggregation",
            )
        elif pd.notna(steps_6h) and steps_6h < ref_steps_6h * 0.5:
            pct_decline = int((1.0 - (steps_6h / ref_steps_6h)) * 100)
            pct_decline = max(0, min(100, pct_decline))
            add_driver(
                drivers,
                "Mobility",
                f"Mobility declined {pct_decline}% over 6h from baseline",
                f"6-Hour steps {steps_6h:.0f} below baseline {ref_steps_6h:.0f}",
                -1,
                "30m_aggregation",
            )
    ...
    if pd.notna(fatigue) and fatigue >= 70:
        add_driver(
            ...
            "Mobility",
            "High Fatigue",
            f"Recent avg fatigue {fatigue:.0f}",
            -1,
            "30m_aggregation",
        )
    ...
    if pd.notna(sleep_available) and int(sleep_available) == 0:
        ...
        sleep_detail = f"Daily sleep {daily_sleep:.0f}min below baseline {sleep_baseline:.0f}min"
        if pd.notna(sleep_baseline) and sleep_baseline < 240:
            sleep_detail += (
        ...
        add_driver(
            drivers,
            "Sleep",
            f"Sleep reduced {int(sleep_deficit * 100)}% below baseline over 24h",
            sleep_detail,
            -2,
            "daily_context",
        )

    # --- V2 Rule A: Failed Physiological Recovery ---
    hr_recovery_rate_15m = row.get("hr_recovery_rate_15m")
    ref_hr = row.get("reference_heart_rate")
    
    # Personalisation: Threshold is either 110, or 35% above their baseline resting HR, whichever is higher.
    hr_threshold = HR_RECOVERY_MIN_THRESHOLD_BPM
    if pd.notna(ref_hr):
        hr_threshold = max(HR_RECOVERY_MIN_THRESHOLD_BPM, ref_hr * 1.35)
        
    if pd.notna(avg_hr) and avg_hr > hr_threshold and pd.notna(hr_recovery_rate_15m) and hr_recovery_rate_15m > HR_RECOVERY_SLOPE_THRESHOLD:
        add_driver(
            drivers,
            "Vitals",
            "Failed physiological recovery",
            f"Heart rate elevated at {avg_hr:.0f}bpm (baseline {ref_hr if pd.notna(ref_hr) else 'N/A'}) with no downward trend for 15 mins",
            -2,
            "30m_aggregation",
        )

    # --- V2 Rule B: Persistent Abnormality Duration (BP Decline) ---
    persistent_sbp_decline_4h = row.get("persistent_sbp_decline_4h")
    sbp_4h_sparse_flag = row.get("sbp_4h_sparse_flag", 0)
    ref_sbp = row.get("reference_systolic_bp")
    
    # Personalisation: Threshold is either 110, or 15% below their baseline SBP, whichever is lower.
    sbp_threshold = SBP_PERSISTENT_MAX_THRESHOLD_MMHG
    if pd.notna(ref_sbp):
        sbp_threshold = min(SBP_PERSISTENT_MAX_THRESHOLD_MMHG, ref_sbp * 0.85)

    if sbp_4h_sparse_flag == 1:
        add_driver(
            drivers,
            "Data Quality",
            "Sparse BP Data",
            "Insufficient valid BP readings over the last 4 hours to calculate persistent decline trend",
            0,
            "30m_aggregation",
        )
    elif pd.notna(avg_sbp) and avg_sbp < sbp_threshold and pd.notna(persistent_sbp_decline_4h) and persistent_sbp_decline_4h < SBP_DECLINE_SLOPE_THRESHOLD:
        add_driver(
            drivers,
            "Vitals",
            "Persistent BP decline",
            f"Systolic BP {avg_sbp:.0f} mmHg has steadily declined over recent hours",
            -1,
            "30m_aggregation",
        )

    # --- V2 Rule C: Suspected False Mobility ---
    step_increment = row.get("step_increment")
    hr_slope_5m = row.get("hr_slope_5m")
    breathing_slope_5m = row.get("breathing_slope_5m")
    stress_slope_5m = row.get("stress_slope_5m")
    
    if pd.notna(step_increment) and step_increment > FALSE_MOBILITY_STEP_THRESHOLD:
        if (pd.notna(hr_slope_5m) and hr_slope_5m <= 0) and \
           (pd.notna(breathing_slope_5m) and breathing_slope_5m <= 0) and \
           (pd.notna(stress_slope_5m) and stress_slope_5m <= 0):
            add_driver(
                drivers,
                "Data Quality",
                "Suspected False Mobility",
                f"High step count ({step_increment:.0f}) detected without accompanying physiological response (HR/Breathing flat)",
                0,
                "30m_aggregation",
            )
        else:
            add_driver(
                drivers,
                "Mobility",
                "Confirmed Activity Burst",
                f"High step count ({step_increment:.0f}) confirmed by multi-domain physiological response",
                0,
                "30m_aggregation",
            )
    ...
    return {
        ...
        "featureContext": {
            ...
            for key in [
                ...
                "coverage_12h",
                "max_gap_minutes",
                "hr_recovery_rate_15m",
                "persistent_sbp_decline_4h",
                "hr_slope_5m",
                "breathing_slope_5m",
                "stress_slope_5m",
                "step_increment",
