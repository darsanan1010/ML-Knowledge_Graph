...
from app.core.rolling_rule_engine_v2 import evaluate_rule_layer
...
def top_public_drivers(record, max_items=3):
    ...
    def _get_domain(t):
        t_low = t.lower()
        if "heart rate" in t_low or "tachycardia" in t_low or "bradycardia" in t_low or "physiological recovery" in t_low:
            return "hr"
        if "blood pressure" in t_low or "bp" in t_low or "systolic" in t_low or "diastolic" in t_low:
            return "bp"
        if "spo2" in t_low or "oxygen" in t_low:
            return "spo2"
        if "sleep" in t_low or "fatigue" in t_low:
            return "sleep"
        if "mobility" in t_low or "activity" in t_low or "step" in t_low:
            return "mobility"
        return t_low
        ...
        if title:
            ...
            if title.lower() not in seen_titles:
                ...
                if domain in seen_domains:
                    continue  # Suppress lower-severity redundant driver for the same domain
    ...
    if not public_drivers:
        ...
        for feat in shap_features:
            ...
            domain = _get_domain(translated_title)
            if domain in seen_domains:
                continue
            seen_domains.add(domain)
            ...
            if raw_feat == "activity_ratio":
                ...
                if pct_decline > 0:
                    ...
                    continue  # Skip positive mobility deviations as fall risk drivers
